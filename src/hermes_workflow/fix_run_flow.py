...
def _has_waveform_exports(project_dir: Path) -> bool:
    """Return True if the project declares waveform exports (config file or
    any metric request carrying waveform_exports)."""
    ...
    *,
    ...
    if real:
        ...
        for point_index, point in enumerate(fixed_points_config.points):
            ...
            # Artifact gate: when waveform exports are configured, every child
            # must have produced its CSV and waveform export manifest. Missing
            # artifacts are child failures (parity with the remote flow).
            if _has_waveform_exports(project_root):
            ...
            for child_issue in child_issues:
                all_issues.append(
                    f"child run {child_issue.testbench_id or ''}/"
                    f"{child_issue.corner_id or ''}: {child_issue.message}"
                )
