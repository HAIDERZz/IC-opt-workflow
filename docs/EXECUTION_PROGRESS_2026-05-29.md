...
- Next recommended action: finish the scoped C-7 command fix, then decide whether the next scoped fix is exported netlist bundle sidecars, Spectre-safe unit formatting, or metric namespace/formula contract.
...
当前 repo 是 /home/zzchen/Agent_virtuoso/EDA_AI_AGENT/ic-auto-opt-workflow，branch 是 plan-a-hermes-file-contract-mvp。请先阅读 docs/CURRENT_TASK_STATE.json、AGENTS.md、docs/NEXT_DEVELOPMENT_LOG_2026-05-31.md、docs/EXECUTION_PROGRESS_2026-05-29.md、docs/COMPACT_RESUME_CHECKPOINT.md。当前活动节点是 Plan C C-12 Task 4 Hermes check/recovery handling，状态为 C-12 Task 4 complete and reviewed；下一步必须先确认是否进入 scoped C-7 adapter command-compatibility fix。不要重新运行真实 Spectre/OCEAN adapter，除非新的 C-7 fix plan 明确要求并获得用户确认。运行或更新任务前先执行 python3 tools/check_development_cadence.py。不要提交或复制本地真实 input.scs/PSF/log 示例，不要让 agent 重写公式，不要用 Python 解析 PSF 或重写 Calculator/OCEAN 公式。
```
