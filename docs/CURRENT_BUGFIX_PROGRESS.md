...
- Removed the positive `legacy execution-agent flag` product CLI path and kept only
...
- Grep for `legacy execution-agent spellings` returns only negative tests that assert the old option is rejected.
