...
- Current next action: request user confirmation before C-12 Task 4 Hermes check/recovery work; do not rerun the adapter.
...
Current next step: ask the user whether to enter C-12 Task 4 Hermes check/recovery work or first scope a C-7 adapter command-compatibility fix. Current state is `Plan C C-12 Task 3 Execution-Agent C-7 adapter invocation`, complete and reviewed, with a structured failed `result_manifest.json` returned by the adapter. Before continuing, read `docs/CURRENT_TASK_STATE.json` and run `python3 tools/check_development_cadence.py`. Do not rerun real Virtuoso/Spectre/OCEAN/SSH/agent/bridge or the C-7 adapter without a new explicit user request. Do not redo Plan A Tasks 1-9 or completed Plan B/C modules unless new review feedback appears.
...
当前 repo 是 /home/zzchen/Agent_virtuoso/EDA_AI_AGENT/ic-auto-opt-workflow，branch 是 plan-a-hermes-file-contract-mvp。当前活动节点是 Plan C C-12 Task 3 Execution-Agent C-7 adapter invocation，状态为 C-12 Task 3 complete and reviewed；下一步必须先向用户确认是否进入 C-12 Task 4 Hermes check/recovery work，或是否先开一个 C-7 adapter command-compatibility fix。运行或更新任务前先执行 python3 tools/check_development_cadence.py。不要重新运行真实 Spectre/OCEAN adapter，不要解析 PSF，不要重写 Calculator/OCEAN 公式。真实 input.scs 示例在 /home/zzchen/Agent_virtuoso/EDA_AI_AGENT/netlist_example 下，仅供本地参考，请勿将其提交到仓库。
```
