...
- Initial replay on `/tmp/ic_auto_opt_optimizer_practice/bridge_test_inv` returned `rise/fall non_scalar` for the template default lower-bound parameters because that candidate's output swing does not reach the 0.9 V threshold required by the approved rise/fall formulas. Treat this as candidate-level infeasibility/performance failure for optimizer penalty logic, not as an OCEAN/formula/layout failure when the known-good baseline candidate still produces scalars.
