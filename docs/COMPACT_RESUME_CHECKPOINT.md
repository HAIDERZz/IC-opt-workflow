...
- Current scope: Fast debug lane for C-7 real-tool adapter compatibility
- Current status: verified-only fast debug lane; C-7 adapter SPECTRE-132 root cause is patched in the working tree with `=log`, and follow-up real-tool contract gaps are recorded separately.
- Current next action: finish the scoped C-7 command fix, then decide whether the next scoped fix is exported netlist bundle sidecars, Spectre-safe unit formatting, or metric namespace/formula contract.
- next_allowed_action: finish the scoped C-7 command fix for =log, then decide whether the next scoped fix is exported netlist bundle sidecars, Spectre-safe unit formatting, or metric namespace/formula contract; do not claim reviewed until review evidence exists
...
Current next step: finish the fast C-7 debug lane cleanup. Current state is `Fast debug lane for C-7 real-tool adapter compatibility`, verified-only. The working-tree code patch fixes Spectre log argument handling (`-log psf/spectre.out` -> `=log psf/spectre.out`). Real-tool debug also exposed exported-netlist sidecar, Spectre unit-format, and metric namespace/formula contract gaps. next_allowed_action: finish the scoped C-7 command fix for =log, then decide whether the next scoped fix is exported netlist bundle sidecars, Spectre-safe unit formatting, or metric namespace/formula contract; do not claim reviewed until review evidence exists. Read `docs/debug/2026-06-03-c7-real-tool-debug-lane.md` before continuing. Before committing, run `python3 -m pytest tests/test_spectre_ocean_adapter.py -q`, `git diff --check`, and `python3 tools/check_development_cadence.py`. Do not commit raw input decks, protected include files, PSF/raw data, full Cadence logs, `docs/OCEAN_DOC_*`, or `docs/toolchain_evidence/`. Do not parse PSF or translate OCEAN formulas in Python.
...
当前 repo 是 /home/zzchen/Agent_virtuoso/EDA_AI_AGENT/ic-auto-opt-workflow，branch 是 plan-a-hermes-file-contract-mvp。当前活动节点是 Fast debug lane for C-7 real-tool adapter compatibility，状态为 verified-only。先读 docs/debug/2026-06-03-c7-real-tool-debug-lane.md。工作树里已有小补丁：C-7 Spectre 命令从 -log psf/spectre.out 改成 =log psf/spectre.out，并同步了 tests/test_spectre_ocean_adapter.py。下一步是完成这个 scoped C-7 command fix 的验证/提交，然后再决定是否开新 scoped fix：exported netlist bundle sidecars、Spectre-safe unit formatting、或 metric namespace/formula contract。不要提交 raw input.scs、ade_e.scs、PSF/raw、完整 Cadence log、docs/OCEAN_DOC_*、docs/toolchain_evidence/。不要用 Python 解析 PSF，不要让 agent 翻译或重写 Calculator/OCEAN 公式。
```
