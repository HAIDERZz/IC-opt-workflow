...
- Current scope: C-22 Execution-Agent Task Package Alignment.
- Current status: C-22 Task 1 is complete, verified-only, and committed in the current HEAD. The generated execution package task now uses generic execution-agent wording and includes the C-20 manifest-level audit rule.
- Current next action: wait for user confirmation before choosing the next narrow real-practice-backed optimizer or execution-agent validation step.
...
6. ic-auto-opt-workflow/docs/superpowers/plans/2026-06-04-execution-agent-task-package-alignment.md
7. ic-auto-opt-workflow/docs/superpowers/plans/2026-06-04-ocean-metric-extraction-retry-policy.md
...
当前 repo 是 /home/zzchen/Agent_virtuoso/EDA_AI_AGENT/ic-auto-opt-workflow，branch 是 plan-a-hermes-file-contract-mvp。C-22 Execution-Agent Task Package Alignment Task 1 已完成，状态 verified-only，并已提交在 current HEAD。它只更新生成的 `execution_package/EXECUTION_TASK.md`：使用 generic execution-agent wording，移除 Claude-specific title，并加入 C-20 证据得到的规则：command exit status alone is not acceptance evidence；manifest-level audit is required for any real-tool run。下一步等待用户确认下一个窄 scoped real-practice-backed optimizer 或 execution-agent validation 范围。必须继续基于 `run-native-turbo --parallel` 和 native `Turbo1.optimize()` 路线；不要写 broad optimizer framework，不要提交 raw input.scs、ade_e.scs、PSF/raw、完整 Cadence log、docs/OCEAN_DOC_*、docs/toolchain_evidence/。不要用 Python 解析 PSF，不要让 agent 翻译或重写 Calculator/OCEAN 公式。
```
