...
- C-11 local/fake controlled smoke 已完成并通过 final gate：它验证 C-9 -> fake C-7-style returned artifacts -> C-5/C-6 checks -> C-8 happy path 和一个 C-10 failure/retry path，且没有运行真实 Virtuoso/Spectre/OCEAN/SSH/agent/bridge。下一步必须是单独的 real-tool/agent practice plan；新的 design spec 获批前不要运行真实工具。
...
C-11 local/fake controlled smoke 已完成。它验证 C-9 -> fake C-7-style returned artifacts -> C-5/C-6 checks -> C-8 happy path 和一个 C-10 failure/retry path，没有运行真实 Virtuoso/Spectre/OCEAN/SSH/agent/bridge。下一步必须另起 real-tool/agent practice plan，并在新的 design spec 获批后再运行真实工具。
