...
- C-11 local smoke design spec 和 implementation plan 已创建：`docs/superpowers/specs/2026-06-03-local-real-run-smoke-design.md`、`docs/superpowers/plans/2026-06-03-local-real-run-smoke.md`。下一步是按 Subagent-Driven 执行 C-11 Task 1 test-only smoke helpers，并在完成后停止报告。C-11 串联 C-9 -> fake C-7-style returned artifacts -> C-5/C-6 checks -> C-8，并包含一个受控 C-10 failure/retry case。C-11 smoke 仍应先使用 fake/local controlled flow，不直接真实接入 Virtuoso/Spectre/OCEAN/agent。
...
C-11 local smoke 是下一步，应保持不先接入真实 Virtuoso/Spectre/OCEAN/agent。
