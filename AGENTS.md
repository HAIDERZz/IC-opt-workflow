...
- Product-shaped optimizer use has distinct states. The implemented shell route is `ic-opt PROJECT_DIR --real`; `hermes-workflow optimize PROJECT_DIR --real` remains the lower-level implementation command. The first Claude CLI slash-skill route is `/ic-opt PROJECT_DIR --real` after installing `claude_skills/ic-opt`, and C-63 proved it on a fresh real Mixer multi-testbench run. Automatic supervisor-agent to execution-agent dispatch is not implemented yet. Do not claim the two-agent product is complete, and do not validate production UX by giving the supervisor a long natural-language prompt that restates the manual. Read `docs/AGENT_INTEGRATION_STATUS.md` before describing the user-facing agent boundary.
