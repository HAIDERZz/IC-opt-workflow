...
- C-70 remote Spectre/OCEAN local parity is accepted. Keep the reusable C-69/C-70 pieces: passwordless SSH like `virtuoso-bridge-lite`, remote project/cache routing, remote doctor, report/history mirroring, local OpenBox/controller/report logic, and remote Spectre/OCEAN execution that reuses the proven local command, cwd, artifact, diagnostic, and manifest semantics. Do not reintroduce hand-built remote Spectre/OCEAN commands/manifests, remote-side OpenBox/product Python installation, or formula/parser changes for remote mode.
