...
- Product-shaped optimizer use has distinct states. The implemented shell route is `ic-opt PROJECT_DIR --real`; `hermes-workflow optimize PROJECT_DIR --real` remains the lower-level implementation command. The Claude CLI slash-skill route is `/ic-opt PROJECT_DIR --real` after installing `claude_skills/ic-opt`; C-64 proved that this route dispatches an independent Claude CLI execution-agent process through `--execution-agent claude` on a fresh real Mixer multi-testbench run. Shell `ic-opt` remains direct by default for operator/debug use. Do not claim Codex or non-Claude two-agent support before a runtime-specific adapter and real handoff drill exist, and do not validate production UX by giving the supervisor a long natural-language prompt that restates the manual. Read `docs/AGENT_INTEGRATION_STATUS.md` before describing the user-facing agent boundary.
