...
- C-69 remote SSH execution is a new design direction, not implemented behavior. Remote mode should keep the optimization project directory on the Linux EDA server with the same local-mode structure, require user-configured passwordless SSH like `virtuoso-bridge-lite`, run OpenBox/controller/report logic on the local workstation, run Spectre/OCEAN on the remote Linux server, keep full artifacts on the remote project, and optionally mirror reports locally. Do not require installing OpenBox or the product Python environment on the EDA server for the remote-mode MVP.
