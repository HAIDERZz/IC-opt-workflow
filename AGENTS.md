...
- Product-shaped optimizer use has two distinct states. The implemented shell route is `ic-opt PROJECT_DIR --real`; `hermes-workflow optimize PROJECT_DIR --real` remains the lower-level implementation command. The real agent-facing slash command `/ic-opt PROJECT_DIR --real` and automatic supervisor-agent to execution-agent dispatch are not implemented yet. Do not claim the two-agent product is complete, and do not validate production UX by giving the supervisor a long natural-language prompt that restates the manual. Read `docs/AGENT_INTEGRATION_STATUS.md` before describing the user-facing agent boundary.
