...
- Product-shaped optimizer use has distinct states. The implemented shell route is `ic-opt PROJECT_DIR --real`; `hermes-workflow optimize PROJECT_DIR --real` remains the lower-level implementation command. The current product-facing agent route is platform-neutral: give the current agent `skills/ic-opt/SKILL.md`, let it run the deterministic `ic-opt` CLI, and let it explain the reports. Do not describe Claude/OpenCode/Codex-specific adapters, independent agent subprocess handoff, or long prompt scripts as the current product boundary unless a new design explicitly restores that route. Release packages should stay free of local development-process docs such as `docs/superpowers/`.
- C-69 remote SSH/product plumbing is implemented, but C-70 is the active correction for remote Spectre/OCEAN local parity. Keep the reusable C-69 pieces: passwordless SSH like `virtuoso-bridge-lite`, remote project/cache routing, remote doctor, report/history mirroring, and local OpenBox/controller/report logic. Do not treat the C-69 remote Spectre/OCEAN adapter as accepted: it hand-built commands/manifests and drifted from the proven local adapter. C-70 must make remote Spectre/OCEAN reuse local command, cwd, artifact, and manifest semantics. Do not require installing OpenBox or the product Python environment on the EDA server for the remote-mode MVP.
