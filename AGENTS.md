...
- Product-shaped optimizer use must converge on `/ic-opt PROJECT_DIR --real` for agent-facing sessions and `ic-opt PROJECT_DIR --real` for shell-facing sessions. `hermes-workflow optimize PROJECT_DIR --real` remains the lower-level implementation command. Do not validate production UX by giving the supervisor a long natural-language prompt that restates the manual.
